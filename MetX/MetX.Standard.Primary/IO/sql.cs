...
namespace MetX.Standard.Primary.IO
{
    ...
    public static class Sql
    {
        ...
        public static int RetrieveSingleIntegerValue(string sql, string connectionName = null,
        ...
        public static bool UpdateNotificationField(string notificationId, string fieldName, string newValue,
            int maxLength)
        {
            var conn = DefaultConnection;
            var cmd = new SqlCommand("SET ARITHABORT ON", conn);
            cmd.ExecuteNonQuery();
            cmd.CommandText = "UPDATE Notification SET " + fieldName + "='" + newValue.Substring(0, maxLength) +
            ...
            if (sqlArray.Count > 0)
            {
                ...
                foreach (var sql in sqlArray)
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        int currSql;
                        sb.Append(
                            "Exception while executing SQL # " + errorAtSql + ": " + sql + "\r\n\r\n" + e + "\r\n");
                        sb.Append("\r\n------------------------------------------------------------------------\r\n");
