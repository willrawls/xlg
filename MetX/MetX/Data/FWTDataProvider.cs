...
namespace MetX.Data
{
    public class FWTDataProvider : DataProvider
    {
        public FWTDataProvider(string connectionString)
        {
        ...
        {
            return new FWTConnection(connectionString);
        }
        ...
        {
            FWTConnection conn = new FWTConnection(connectionString);
            conn.Open();
            FWTCommand cmd = new FWTCommand(qry.CommandSql, conn);
        ...
        {
            using (FWTConnection conn = new FWTConnection(connectionString))
            {
                FWTCommand cmd = new FWTCommand(qry.CommandSql, conn);
                conn.Open();
        ...
        {
            FWTCommand cmd = new FWTCommand(qry.CommandSql);
            return cmd;
        }

        #region Schema Bits
        const string TABLE_SQL = "SELECT name As Name FROM sysobjects WHERE type='U'";
        const string TABLE_COLUMN_SQL = "SELECT syscolumns.name AS ColumnName, colid AS OrdinalPosition, status AS IsNullable, syscolumns.type AS DataType, length AS MaxLength, 0 As IsIdentity FROM syscolumns JOIN sysobjects ON syscolumns.id=sysobjects.id WHERE sysobjects.name=@tblName";
